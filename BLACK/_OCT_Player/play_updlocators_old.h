...
//if (row > 7){
//d_iag_printf( "GNT: nt from set:%d\n", target_page->Track[row]->trigger_set_ndx );
////	for (i=0; i<MATRIX_NROF_ROWS; i++){
////		d_iag_printf( "trigger:%d val:%d\n", i, target_page->Track[row]->trigger[i] );
////	}
//}
...
unsigned char get_next_trigger_set_new( Pagestruct* target_page, Trackstruct* target_track ){
	...
	// Jump over potentially skipped steps at new resulting position
	result = jump_skipped_position_new( target_page, row, candidate, direction );
...
unsigned char get_next_trackposition( Pagestruct* target_page, unsigned char row ){

	unsigned int old_offset	=	0;
	signed   int result		=	0;	// Goes below 0 (becomes =-1) at some point in time
	unsigned int locator 	= 	target_page->Track[row]->attribute[ATTR_LOCATOR];

//if (row > 7)
//d_iag_printf( "GNTP: row:%d start locator:%d ts:%d\n", 
//	row, locator, target_page->Track[row]->trigger_set_ndx );
	...
	if (	( locator == 0 )
		||	( target_page->Track[row]->trigger_set_ndx == 0 )
		){
	
		target_page->Track[row]->trigger_set_ndx 
			= get_next_trigger_set( target_page, row );

		// Refill trigger stack of track - refill I
		memcpy( &target_page->Track[row]->trigger, 
				&direction_repository[ target_page->Track[row]->attribute[ATTR_DIRECTION] ]
				.trigger_set[ target_page->Track[row]->trigger_set_ndx ]
				.trigger,
				10 );
...
//if (row > 7)
//d_iag_printf( "GNTP: row:%d trigger_set:%d\n", row, target_page->Track[row]->trigger_set_ndx );
	...
	if ( result == 0 ){
		...
		if ( target_page->Track[row]->trigger_set_ndx == 0 ){
...
//			if (row > 7)
//			d_iag_printf( "GNTP: row:%d new trigger_set_ndx:%d trigger:%d\n", 
//				row,
//				target_page->Track[row]->trigger_set_ndx,
//				result );
...
//	if (row > 7)
//		d_iag_printf( "GNTP: final: row:%d t_set:%d t:%d\n", 
//			row,
//			target_page->Track[row]->trigger_set_ndx,
//			result );
...
unsigned char advance_track_locator( Pagestruct* target_page, unsigned char row ){

	unsigned char new_pos 	= 0;
	unsigned char j 		= 0;
	unsigned char temp		= 0;
	unsigned char result 	= EMPTY;		// Signals whether track wrapping has taken place
	
	Trackstruct* this_track = target_page->Track[row];

	// Do not process tracks of a chain, which are not playing
	if ( this_track->chain_data[PLAY] != this_track ){
	
		return EMPTY;
	}

//if (row > 7)
//d_iag_printf( "ATL: row:%d new_pos: wanted!\n", row );


	// Get the new position of the locator for the track
	new_pos = get_next_trackposition( target_page, row );

//if (row > 7)
//d_iag_printf( "ATL: row:%d new_pos:%d\n\n", row, new_pos );
	...
	if ( new_pos == 0 ){
		...
		if ( this_track->chain_data[NEXT] == this_track ){
		...
		else if ( this_track->chain_data[NEXT] != this_track ){

			// Make the NEXT track the PLAY track in the whole track chain
			update_trackchain_PLAY( target_page, this_track, this_track->chain_data[NEXT] );

			// Now: need to update the locator on the new PLAY track
			temp = row_of_track( target_page, this_track->chain_data[PLAY] );
			
//if (row > 7)
//d_iag_printf( "ATL: wrap-> NEXT row:%d\n", temp );

			// ..but only if it was looked at already.. which we know by comparing the rows
			if ( temp < row ){

				advance_track_locator( target_page, temp );	
				result = temp;
			}
			// The new play track is this track, because of skips..
			else if ( temp == row ){
				
				// Get the new position after the virtual wrap
				new_pos = get_next_trackposition( target_page, row );
			}
	...
	this_track->attribute[ATTR_LOCATOR] = new_pos;

//if (row > 7)
//d_iag_printf( "ATL: row:%d final pos:%d\n", row, new_pos );
...
// Advance the locator of one track. Returns the index of the track wrapped to.
Trackstruct* advance_track_locator_new( Pagestruct* target_page, Trackstruct* target_track ){

	unsigned char new_pos 	= 0;
	unsigned char j 		= 0;
	unsigned char temp		= 0;
	unsigned char row 		= row_of_track( target_page, target_track );	
	Trackstruct* this_track = target_track;
	...
	if ( new_pos == 0 ){
		...
		// SWITCH to the next track in the chain (track is part of a chain)
			...
			if ( temp < row ){
				...
				advance_track_locator_new( target_page, temp );	
				result = temp;
			}
			// The new play track is this track, because of skips..
			else if ( temp == row ){
...
Trackstruct* advance_track_TTC_new( Pagestruct* target_page, Trackstruct* target_track ){
	...
	if ( 	(target_track->TTC 	== 1 )
		...
		// Universal track locator advancement - may return the next track in chain
		target_track = advance_track_locator_new( target_track );
		...
		if (	( target_page->track_REC_pattern != 0 )
			...
			target_page->track_REC_pattern = 
				( 1 <<  row_of_track( target_page, target_page->Track[row]->chain_data[PLAY] ) );
		}
	}
