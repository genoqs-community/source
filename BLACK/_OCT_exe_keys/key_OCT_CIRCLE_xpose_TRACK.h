		...
		for ( row=0; row < MATRIX_NROF_ROWS; row++ ){
			...
			switch( j ){
				...
				case 0:
					...
					if (( VIEWER_page->Track[row]->attribute[ATTR_PITCH] % 12 ) == 0 ){

						// Make sure we stay within bounds
						if ( VIEWER_page->Track[row]->attribute[ATTR_PITCH] >= (TRACK_MIN_PITCH+12) ){ 
						...
						VIEWER_page->Track[row]->attribute[ATTR_PITCH] =
							( VIEWER_page->Track[row]->attribute[ATTR_PITCH] / 12 )*12 + j;
					}
