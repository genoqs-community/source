	...
	// ..select the page for play in preselection or zoom into it (doubleclick)
		...
		if (	( DOUBLE_CLICK_TARGET == keyNdx ) 
			...
			VIEWER_page = &Page_repository[ GRID_p_selection[ row_of(keyNdx) ]->pageNdx ];	
			// Move the cursor for the grid
			GRID_CURSOR = VIEWER_page->pageNdx;

			// Put the track into the track selection
			VIEWER_page->trackSelection ^= mirror( 1 << (column_of( keyNdx )-3), 10 );

			// Zoom into the VIEWER_page (implicitly) and the track there		
