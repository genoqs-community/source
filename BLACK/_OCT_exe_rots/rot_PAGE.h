...
//	#include "rot_editorblock.h"
	...
	rot_exe_EDIT( rotNdx, direction, VIEWER_page );
}
	...
	if ( rotNdx == 0){
		...
		if ( GRID_bigknob_SELECT == OFF ){
			...
			if ( my_bit_cardinality( VIEWER_page->trackSelection ) == 1 ){
				...
				modify_parameter( 	&VIEWER_page->Track[ my_bit2ndx( VIEWER_page->trackSelection )]->program_change, 
		...
		// Jump across the playing pages
			...
			// Jump the VIEWER_page according to SELECT
			switch( direction ){
				...
				case INC:
					...
					for ( i=GRID_NROF_BANKS+1; i>0; i-- ){
						...
						if ( 	( j == TRUE )
							...
							VIEWER_page = GRID_p_selection[i-1];
						...
						if ( GRID_p_selection[i-1] == VIEWER_page ){
...
#include "rot_mixerblock.h"
	...
	//
		...
		// Operate on the track selection
			if ( VIEWER_page->trackSelection != 0 ){
			...
			VIEWER_page->editAttribute = rotNdx;
			...
			for (row=0; row < MATRIX_NROF_ROWS; row++) {
				...
				for (col=0; col < MATRIX_NROF_COLUMNS; col++) {
					...
					if (Step_get_status( VIEWER_page->Step[row][col], STEPSTAT_SELECT ) == ON){
						...
						apply_rotary_to_step( rotNdx, direction, VIEWER_page->Step[row][col] );
		...
		// Entering Roland Fleming mode: make sure that step is on, and edit its atribute 
			...
			if ( key_pressed == KEY_ZOOM_PAGE ){
				...
				switch( rotNdx ){
					...
					// VEL
					case 1:
						modify_parameter( 	&VIEWER_page->attribute[ATTR_VELOCITY], 
											PAGE_MIN_VELOCITY, PAGE_MAX_VELOCITY, direction, ON, FIXED );
						break;

					// PIT
					case 2:
//							modify_parameter( 	&VIEWER_page->attribute[ATTR_PITCH], 
//												PAGE_MIN_PITCH, PAGE_MAX_PITCH, direction, ON, FIXED );
						modify_parameter( 	&Page_repository[GRID_CURSOR].attribute[ATTR_PITCH], 
											PAGE_MIN_PITCH, PAGE_MAX_PITCH, direction, ON, FIXED );
						break;
		...
		// MIX2EDIT is when the EDIT block acts similarly to the MIX block
		else if ( VIEWER_page->editorMode == MIX2EDIT ) {
			...
			switch ( VIEWER_page->attr_mix2edit ){
				...
				case ATTR_LENGTH:
					modify_signed_parameter 
						( &VIEWER_page->Track[i]->LEN_factor, 
							VIEWER_page->attr_mix2edit_MIN, 
							VIEWER_page->attr_mix2edit_MAX, 
							direction, OFF, FIXED);
					break;
				...
				default:
					...
					modify_signed_parameter 
						( &VIEWER_page->Track[i]->attribute[VIEWER_page->attr_mix2edit], 
							VIEWER_page->attr_mix2edit_MIN, 
							VIEWER_page->attr_mix2edit_MAX, 
							direction, OFF, FIXED);
					break;
		...
		// Needs cleanup at some point..
		else if ( VIEWER_page->editorMode == CONTROL ) {
			...
			if ( VIEWER_page->Track[i]->attribute[ATTR_MIDICC] != MIDICC_NONE ){
				...
				modify_signed_parameter 
					(&VIEWER_page->Track[i]->attribute[ATTR_CCAMNT], 
				...
				MIDI_send( 	MIDI_CC,
							...
							VIEWER_page->Track[i]->attribute[ATTR_MIDICC],					
							VIEWER_page->Track[i]->attribute[ATTR_CCAMNT] 	);
