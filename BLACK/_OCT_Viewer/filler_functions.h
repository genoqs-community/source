...
void MIR_write_dot( 			unsigned int button_ndx, 
	...
	else if ( (button_ndx > 10) && (button_ndx < 186) ) { 
		switch (button_ndx) {
...
// Writes the lauflicht of trackNdx into row of trackNdx - used in Track editing etc..
void MIR_write_lauflicht_track( unsigned char trackNdx ) {
	...
	locator = VIEWER_page->Track[trackNdx]->attribute[ATTR_LOCATOR];
	...
	if ( 	(( MIR[side][trackNdx][MIR_RED] & (1 << (locator - offset)) ) >  0)
		&&	(( MIR[side][trackNdx][MIR_GREEN]   & (1 << (locator - offset)) ) == 0) )  {
	
		// Turn off RED bit in MIR, and BLINK too since we're at it anyways..
		MIR[side][trackNdx][MIR_RED] 	&= 0xff ^ (1 << (locator - offset));	
		MIR[side][trackNdx][MIR_BLINK] 	&= 0xff ^ (1 << (locator - offset));
	}
	else {

		// This locator needs to be more prevalent and needs to be single RED
		MIR[side][trackNdx][MIR_RED] 	|= (1 << (locator - offset));
	
		// Turn off all BLINK and GREEN bits in the MIR
		MIR[side][trackNdx][MIR_BLINK] 	&= 0xff ^ (1 << (locator - offset));
		MIR[side][trackNdx][MIR_GREEN] 	&= 0xff ^ (1 << (locator - offset));
	}
}
...
void MIR_write_lauflicht () {
	...
	for (i=0; i<MATRIX_NROF_ROWS; i++) {
		...
		locator = VIEWER_page->Track[i]->attribute[ATTR_LOCATOR];
...
//		// This is now customized for the GRID scene utilization
//		switch( GRID_status ){
//			case GRID_EXPORT:
//				MIR_write_dot( circleNrField[number], MIR_BLINK	);	
//				break;
//
//			default:
//				// Dot the number into the Circle area
//				MIR_write_dot( circleNrField[number], MIR_RED 	);
//				MIR_write_dot( circleNrField[number], MIR_GREEN );
//				break;
//		}
...
// Writes a factor (range 0-10) into the numeric circle quadrant
...
// Writes a step length into the given matrix row (Horizontal) using the 
// following representation:
// GREEN Bar shows how many full 1/16ths the step length is (1/16 equiv. to 24TTC ticks)
// RED LED shows the "Gate" length in addition to the full 1/16ths.
...
void MIR_write_numeric_H( signed char number, unsigned char target_row ) {
	...
	if (tens <= 8) {
		...
		if ( value_sign == NEG ){
			MIR[LHS][target_row][MIR_BLINK] |= bitpattern;
		}
	}
	...
	if (tens >= 9) {
		...
		if ( value_sign == NEG ){
			MIR[LHS][target_row][MIR_BLINK] |= 0xFF;
		}
		...
		if ( value_sign == NEG ){
			MIR[RHS][target_row][MIR_BLINK] |= bitpattern;
		}
	...
	if (ones <= 8) {
		...
		if (ones != tens ){
			...
			if ( value_sign == NEG ){
				MIR[LHS][target_row][MIR_BLINK] &= ( 0xFF ^ (1 << (ones-1)) );
			}
		}
	}
	...
	if (ones > 8) {
		...
		if (ones != tens ){
			...
			if ( value_sign == NEG ){
				MIR[RHS][target_row][MIR_BLINK] &= ( 0xFF ^ (1 << 0) );
			}
		}
...
void MIR_show_groovebar_V( unsigned char row ){
	...
	for ( col = 0; col < MATRIX_NROF_COLUMNS; col ++ ){
		...
		total = (unsigned char) VIEWER_page->Step[row][col]->attribute[ATTR_GROOVE];
...
void MIR_show_lengthbar_V( unsigned char row ){
	...
	for (col=0; col<MATRIX_NROF_COLUMNS; col++) {
		...
		total = (unsigned char) VIEWER_page->Step[row][col]->attribute[ATTR_LENGTH];
...
//// Using the given row / track of the VIEWER_page
...
//					value 	= 	0;
//		
//	// Get the groove values of the steps in track [row] and display them
//	for ( col=0; col<MATRIX_NROF_COLUMNS; col++ ){
//
//		// Get the real value before computing the other offsets, etc.
//		value = VIEWER_page->Step[row][col]->attribute[ATTR_START];
//		
//		if ( value >= 6 ){									
//			// Normalize for further calculations
//			value = value - 6;
//
//			// Interval mapping (Book p.213) Source [0..6], Target [0..4]
//			// Show the small offsets as well - prevent losing the 1 value
//			if ( value == 1 ){
//				value = 2;
//			} 
//			value = (value * 4) / 6;
//
//			// Place the orange dot 
//			MIR_point_numeric( col+1, 4-value, MIR_RED  	 	);
//			MIR_point_numeric( col+1, 4-value, MIR_GREEN 	 	);
//
//			// Place RED dots under the orange dot
//			for ( i=0; i<value; i++ ){
//				MIR_point_numeric( col+1, 4-i, MIR_RED	);
//			}
//		} // value >= 6
//
//		else if ( value < 6 ){								
...
//			MIR_point_numeric( col+1, 4+value, MIR_GREEN 	 	);
//
//			// Place RED dots above the orange dot
//			for ( i=0; i<value; i++ ){
//				MIR_point_numeric( col+1, 4+i, MIR_RED	);
//			}
//		} // value < 6
...
void MIR_show_startbar_V( unsigned char row ){
	...
	for ( col=0; col < MATRIX_NROF_COLUMNS; col++ ){
		...
		// Get the real value before computing the other offsets, etc.
		total = VIEWER_page->Step[row][col]->attribute[ATTR_START];
...
//	unsigned char tens_color = MIR_RED;
	...
	for (col=0; col < MATRIX_NROF_COLUMNS; col++) {
		...
		else if ( total <= (unsigned char) STEP_MAX_LENGTH ){
			for (j=0; j < 5; j++) {
				MIR[side][j][MIR_RED] 	|= (1<<(col % 8));	
			} 
		}	
...
void MIR_write_numeric_V_zeroline_PIT(	unsigned char center_line,
	...
	for (col=0; col < MATRIX_NROF_COLUMNS; col++) {
		...
		total = VIEWER_page->Step[row][col]->attribute[attr+1];
...
//// Supported interval is [-49..+49] - this determines the max offset value
//void MIR_write_numeric_V_zeroline(	unsigned char center_line,
//									signed char min_val,
//									signed char max_val,
//									unsigned char row, 
//									unsigned char attr ){
...
//
//		// *** TENS DISPLAY
//		for (j=center_line; j<(center_line+tens); j++) {
//
//			switch( sign ){
//
//				case POS:
...
//		} // tens
//			
//		// Make the orange tip - need to use the last valid j:
//		j--;
//		switch( sign ){
//
//			case POS:
//				MIR[side][8-j][MIR_GREEN] 	|= (1<<(col % 8));
//				break;
//
//			case NEG:
//				MIR[side][  j][MIR_GREEN] 	|= (1<<(col % 8));	
...
void MIR_write_numeric_V_MCC ( Pagestruct* target_page, unsigned char row ) {
	...
	for (col=0; col < MATRIX_NROF_COLUMNS; col++) {
		...
		total = target_page->Step[row][col]->attribute[ ATTR_MIDICC ];
...
void MIR_write_numeric_V (unsigned char row, unsigned char attr) {
	...
	unsigned int trackpattern = Page_get_trackpattern( VIEWER_page, row );
...
unsigned int attributes_of (unsigned char obj_type, int obj_target) {
	...
	for (attr=0; attr<10; attr++){
		if (PANEL_matrix[obj_type][attr] > 0){
			bitpattern |= (1 << attr);
		}
	}						
...
void display_Track_TEMPOMUL( Trackstruct* target_track ){
	...
	switch( target_track->attribute[ATTR_TEMPOMUL] ){
		...
		case 1:
			...
			switch( target_track->attribute[ATTR_TEMPOMUL_SKIP] & 0x0F ){
