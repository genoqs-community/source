...
void show (unsigned int target, unsigned int content) {
	...
	Pagestruct* target_page = VIEWER_page;
			...
			if ( 	( i == MIXTGT_USR1 )
				...
				if (	( zoom_level == zoomPAGE )
					...
					if ( GRID_MAP0_status == ON ){
						// Show that it is ongoing
						MIR_write_dot( LED_MIXTGT_USR0, MIR_RED );
					}
					else{
						// MIR_write_dot( LED_MIXTGT_USR0, MIR_GREEN );
						// MIR_write_dot( LED_MIXTGT_USR0, MIR_BLINK );
						MIR_write_dot( LED_MIXTGT_USR0, MIR_GREEN );
					}
				}
			}
...
//
//			switch (VIEWER_page->tempoMultiplier) {
//				case 1:
...
//
//				
//					j = my_shift_bitpattern( 	VIEWER_page->scaleNotes[0], 12, INC,
//												my_bit2ndx( VIEWER_page->scaleLead[0] )+1);
//					
//					switch( j ) {
//
//						case SCALE_SIG_PEN:
...
//
//
//					// Now light orange the one that is selected
//					// MIR_write_dot( LED_SCALE
//
		...
		case ELE_CHORD_SELECTOR:
			...
			switch( content ){
				...
				case CHORD_SIZE:
					...
					// MIR_write_dot( 258 - VIEWER_page->chordSize + 1, MIR_RED   );
					// MIR_write_dot( 258 - VIEWER_page->chordSize + 1, MIR_GREEN );
				...
				case CHORD_SIZE_TRACK:
					...
					i = Track_get_MISC( VIEWER_page->Track[ my_bit2ndx( VIEWER_page->trackSelection ) ],
						CHORD_BIT );
...
//					MIR_write_dot( VIEWER_page->cadence->key[ VIEWER_page->cadence->locator ], MIR_GREEN );
//					MIR_write_dot( VIEWER_page->cadence->key[ VIEWER_page->cadence->locator ], MIR_RED );
					break;
			}
			break;
