...
unsigned char record_chord_arp_to_track( Pagestruct* target_page,
	...
	if ( G_track_rec_bit == OFF ){ // rehearse

		if ( Step_get_status(SOLO_assistant_page->Step[ARP_TRACK][arpStep], STEPSTAT_TOGGLE) == ON ){

			PLAYER_preview_step( SOLO_assistant_page, ARP_TRACK, arpStep ); // play the sound with the note length
		}
		return OFF;
	}
	else { // record
