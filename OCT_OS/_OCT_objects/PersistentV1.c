 ...
 */
void PersistentV1_GridExport( GridPersistentV1* targetGridPt )
{
	int setIx, bankIx, rowIx;

	//d_iag_printf( "grid-export to %p size=%ld ", targetGridPt, sizeof(GridPersistentV1));

	// Init target grid buffer.
	PersistentV1_GridInit( targetGridPt );


	// Set meta data fields.
	targetGridPt->magic1 		= PERSISTENT_GRID_MAGIC1;
	targetGridPt->magic2 		= PERSISTENT_GRID_MAGIC2;
	targetGridPt->sizeBt 		= sizeof(GridPersistentV1);
	targetGridPt->version 		= 1;
	...
	PersistentV1_PageExport( GRID_assistant_page, &targetGridPt->assistant_page );



	// Calc and set checksum.
	card8* curPt = (card8*) targetGridPt;
	card8* endPtEx = curPt + sizeof(GridPersistentV1);
	card16 checkSum = 0;
	while ( curPt < endPtEx ) {
		checkSum += *curPt++;
	}
 ...
 * @author Gabriel Seher, Robert van der Kamp
 */
void PersistentV1_GridImport( const GridPersistentV1* sourceGridPt )
{
	int setIx, bankIx, rowIx;
	...
	// Check object size.
	if ( sourceGridPt->sizeBt != sizeof(GridPersistentV1)) {
		return;
	}

	// Check format version number.
	if ( sourceGridPt->version != 1 ) {
		return;
	}
	...
	const card8* endPtEx 	= curPt + sizeof(GridPersistentV1);
	card16 myCheckSum 		= 0;
	...
	PersistentV1_PageImport( &sourceGridPt->assistant_page, GRID_assistant_page );
}
...
void PersistentV1_PageExport( const Pagestruct* sourcePagePt, PagePersistentV1* targetPagePt)
{
	unsigned char i=0, j=0, k=0;

	PersistentV1_PageInit( targetPagePt );

	// Set meta data fields.
	targetPagePt->magic1 		= PERSISTENT_PAGE_MAGIC1;
	targetPagePt->magic2 		= PERSISTENT_PAGE_MAGIC2;
	targetPagePt->sizeBt 		= sizeof(PagePersistentV1);
	targetPagePt->version 		= 1;
	targetPagePt->pageCheckSum 	= 0;

	// Export the tracks and steps.
	for (i=0; i<MATRIX_NROF_ROWS; i++) {
		PersistentV1_TrackAndStepExport(sourcePagePt, i, targetPagePt);
	}
	...
	card8* endPtEx = curPt + sizeof(PagePersistentV1);
	card16 checkSum = 0;
	while ( curPt < endPtEx ) {
		checkSum += *curPt++;
	}
 ...
 * @author Gabriel Seher, Robert van der Kamp
 */
void PersistentV1_PageImport(const PagePersistentV1* sourcePagePt, Pagestruct* targetPagePt)
{
	unsigned char i=0, j=0, k=0;
	...
	// Check object size.
	if ( sourcePagePt->sizeBt != sizeof(PagePersistentV1)) {
		return;
	}

	// Check format version number.
	if ( sourcePagePt->version != 1 ) {
		return;
	}
	...
	const card8* endPtEx 	= curPt + sizeof(PagePersistentV1);
	card16 myCheckSum 		= 0;
	...
	for (i=0; i<MATRIX_NROF_ROWS; i++) {
		PersistentV1_TrackAndStepImport(targetPagePt, i, sourcePagePt);
	}
 ...
 */
void PersistentV1_GridInit( GridPersistentV1* gridPt )
{
	memset( gridPt, 0, sizeof( GridPersistentV1 ) );
}
 ...
 * @author Gabriel Seher, Robert van der Kamp
 */
void PersistentV1_PageInit( PagePersistentV1* pagePt )
{
	unsigned char i=0, j=0;

	memset( pagePt, 0, sizeof(PagePersistentV1) );

	// Init the tracks and the steps
	for (i=0; i<MATRIX_NROF_ROWS; i++) {

		// Init the tracks.
		PersistentV1_TrackInit( &pagePt->Track[i] );

		// Init the steps
		for (j=0; j<MATRIX_NROF_COLUMNS; j++) {
			PersistentV1_StepInit( &pagePt->Step[i][j]);
		}
	}
 ...
 * @author Gabriel Seher, Robert van der Kamp
 */
void PersistentV1_TrackAndStepExport(const Pagestruct* sourcePagePt, card32 source_row, PagePersistentV1* targetPagePt)
{
	unsigned int i=0;

	const Trackstruct* sourceTrackPt = sourcePagePt->Track[source_row];
	TrackPersistentV1* targetTrackPt = &targetPagePt->Track[source_row];
	...
	for (i=0; i<MATRIX_NROF_COLUMNS; i++) {
		PersistentV1_StepExport( sourcePagePt->Step[source_row][i],	&targetPagePt->Step[source_row][i]);
	}
}
 ...
 * @author Gabriel Seher, Robert van der Kamp
 */
void PersistentV1_TrackAndStepImport( Pagestruct* targetPagePt, card32 source_row, const PagePersistentV1* sourcePagePt )
{
	unsigned int i=0;

	const TrackPersistentV1* sourceTrackPt = &sourcePagePt->Track[source_row];
	...
	// Note that the Trackstruct::hyper and gatePosition members were introduced with the PersistentV2 format.
	// Importing tracks from the PersistentV1 format thus simply initialize these members to default value.
	...
	// STEPS
	for (i=0; i<MATRIX_NROF_COLUMNS; i++) {
		PersistentV1_StepImport( &sourcePagePt->Step[source_row][i], targetPagePt->Step[source_row][i]);
	}
 ...
 * @author Robert van der Kamp
 */
void PersistentV1_TrackInit( TrackPersistentV1* targetTrackPt )
{
	memset( targetTrackPt, 0, sizeof(TrackPersistentV1) );
}
 ...
 * @author Gabriel Seher, Robert van der Kamp
 */
void PersistentV1_StepInit( StepPersistentV1* targetStepPt )
{
	memset( targetStepPt, 0, sizeof(StepPersistentV1) );
 ...
 * @author Gabriel Seher, Robert van der Kamp
 */
void PersistentV1_StepExport( const Stepstruct* sourceStepPt, StepPersistentV1* targetStepPt ){
 ...
 * @author Gabriel Seher, Robert van der Kamp
 */
void PersistentV1_StepImport( const StepPersistentV1* sourceStepPt, Stepstruct* targetStepPt )
{
	targetStepPt->event_data = sourceStepPt->event_data;
	...
	// Note that the Stepstruct::hyperTrack_ndx was introduced with the PersistentV2 format.
	// Importing steps from the PersistentV1 format thus simply initialize this member to default value.
