		...
		case PASSIVE:
			...
			if ( target_page->stepSelection == 1 ){
				...
				show_pitch_in_circle( 
					target_page->Track[target_page->stepSelectionSingleRow]->attr_PIT +
					target_page->Step[ target_page->stepSelectionSingleRow ]
									 [ target_page->stepSelectionSingleCol ]->attr_PIT, ON );

				if ( PHRASE_TIMER == ON ) {
					PhraseShowPhraseNum( target_page->Step[target_page->stepSelectionSingleRow][target_page->stepSelectionSingleCol]->phrase_num );
				}
				else {
					show_velocity_in_circle(
							normalize( 
									target_page->Track[target_page->stepSelectionSingleRow]->attr_VEL +
									target_page->Step[ target_page->stepSelectionSingleRow ]
									                   [ target_page->stepSelectionSingleCol ]->attr_VEL,
									                   0, 127 ) );
				}
			}
