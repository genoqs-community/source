...
void MIR_write_lauflicht_track( unsigned char trackNdx ) {
	...
	locator = target_page->Track[trackNdx]->attribute[ATTR_LOCATOR];
...
void MIR_write_lauflicht () {
	...
	for (i=0; i<MATRIX_NROF_ROWS; i++) {
		...
		locator = target_page->Track[i]->attribute[ATTR_LOCATOR];
...
void MIR_show_groovebar_V( unsigned char row ){
	...
	for ( col = 0; col < MATRIX_NROF_COLUMNS; col ++ ){
		...
		total = (unsigned char) target_page->Step[row][col]->attribute[ATTR_GROOVE];
...
void MIR_show_lengthbar_V( unsigned char row ){
	...
	for (col=0; col<MATRIX_NROF_COLUMNS; col++) {
		...
		total = (unsigned char) target_page->Step[row][col]->attribute[ATTR_LENGTH];
...
void MIR_show_startbar_V( unsigned char row ){
	...
	for ( col=0; col < MATRIX_NROF_COLUMNS; col++ ){
		...
		total = target_page->Step[row][col]->attribute[ATTR_START];
...
void MIR_write_numeric_V_zeroline_MCC_BENDER(	unsigned char 	center_line,
	...
	for (col=0; col < MATRIX_NROF_COLUMNS; col++) {
		...
		// Do not bother with steps that carry no data
		...
		// Fetch the total attribute value
		total = target_page->Step[row][col]->attribute[attr+1] - 64;
