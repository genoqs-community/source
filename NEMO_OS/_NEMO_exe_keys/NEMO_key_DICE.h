...
void key_exec_DICE( unsigned short keyNdx ){
	...
	if (	(( keyNdx >= 1 ) && ( keyNdx <= 10 ))){
		if ( G_run_bit == ON )
		{
			dice_synced_attr 	= ATTR_DICE_TOGGLE;
	...
	if (	( 	( keyNdx >= 11 ) && ( keyNdx <= 185 )
		...
		if ( (( keyNdx - NEMO_ROW_I ) % 11) == 0) {
			...
			if( DICE_bank->mixAttribute == ATTR_DICE_DCK ) {
				switch( col ){
					case 3: case 4: case 5: case 6:		j = col - 2;	break;
					case 7: case 8: 					j = DICE_DEF_TEMPO_NUDGE;	break;
					case 9: case 10: case 11: case 12:	j = col - 3;	break;
					default: return;
				}
				...
				if ( G_run_bit == OFF ) {
					...
					Dice_set_TEMPOMUL( target_dice, j );
				} else {
					dice_synced_attr = ATTR_DICE_TEMPO_CHANGE;
		...
		if ( (( keyNdx - NEMO_ROW_II ) % 11) == 0) {
			...
			DICE_bank->mixAttribute = target_dice->attr_STATUS;

			if( col > 8 && col < 14 ) {
				if ( G_run_bit == ON ) {
					dice_synced_attr 	= ATTR_DICE_LENGTH;
				...
				if ((DOUBLE_CLICK_TARGET == keyNdx)
					...
					j = 16 - col;
					if ( G_run_bit == ON ) {
						dice_synced_attr = ATTR_DICE_FLR;
						dice_synced_value = j;
					} else {
						// Tempo multiplier ( 15 = global sync, 16 = grid sync ).
						Dice_set_MISC( target_dice, j );
					}
				...
				// SINGLE CLICK SCENARIO
						...
						cyg_alarm_initialize(
								...
								DOUBLE_CLICK_ALARM_TIME );
						// ATTR_DICE_FLR
						DICE_bank->mixAttribute = ATTR_DICE_FLR;

				} // Single click scenario
			} else if( col == 15 ) {
				// ATTR_DICE_DCK
				DICE_bank->mixAttribute = ATTR_DICE_DCK;
			}
		}
		...
		if ( (( keyNdx - NEMO_ROW_III ) % 11 ) == 0 ){
			...
			if ( G_run_bit == ON )
			{
				dice_synced_attr 	= ATTR_DICE_START;
				dice_synced_value 	= col;
		...
		if ( (( keyNdx - NEMO_ROW_IV ) % 11) == 0) {
			...
			if ( col < 10 ){
				...
				assign_dice_flow_shape( col, target_dice->attr_STATUS, DICE_bank, DICE_bank->mixTarget );
				target_dice->flow_shape[ target_dice->attr_STATUS ] = col;

				if( DICE_bank->mixTarget == 0 )
				{
					// Reset factor.
	...
	// MIX MASTER KEY (Chain Dive Mode)
	if (	( keyNdx == KEY_MIX_MASTER ) ){
		// Toggles the quantise mode for DICE
		Dice_set_MISC( target_dice, DICE_CHAIN_FOLLOW );
	}
	...
	if ( keyNdx == KEY_ALIGN && G_run_bit == ON ) {
		dice_synced_attr 	= ATTR_DICE_ALIGN;
