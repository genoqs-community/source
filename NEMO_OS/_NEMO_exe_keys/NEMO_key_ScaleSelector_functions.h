...
//_________________________________________________________________________________________
//
// Key execution snippet for the Scale selection. May be reused as necessary
...
void key_ScaleSelector_functions( unsigned int keyNdx, Pagestruct* target_page ){
	...
	// Transform the octave keypress and interpret octave press
	if ( ( keyNdx > 32 ) && (keyNdx < 158) ){
		
		// Decide which row we're on
		temp = keyNdx % 11;

		//
		// OCTAVE CIRCLE: Key is in the octave circle
		//
		if ( 	((keyNdx-temp) == KEY_NOTE_C	) ||
				((keyNdx-temp) == KEY_NOTE_Cis	) ||
				((keyNdx-temp) == KEY_NOTE_D	) ||
				((keyNdx-temp) == KEY_NOTE_Dis	) ||
				((keyNdx-temp) == KEY_NOTE_E	) ||
				((keyNdx-temp) == KEY_NOTE_F	) ||
				((keyNdx-temp) == KEY_NOTE_Fis	) ||
				((keyNdx-temp) == KEY_NOTE_G	) ||
				((keyNdx-temp) == KEY_NOTE_Gis	) ||
				((keyNdx-temp) == KEY_NOTE_A	) ||
				((keyNdx-temp) == KEY_NOTE_Ais	) ||
				((keyNdx-temp) == KEY_NOTE_B	) 		
			){
			 
			// Transform key Index to a note index and send it to the scale modifier
			modify_scale_composition( target_page, keyNdx_to_noteNdx( keyNdx-temp ), temp+1 );
		} 
	}
