	...
	switch( rotNdx ){
		...
		case 0: case ROT_PIT: case ROT_VEL: case ROT_LEN: case ROT_STA:
			...
			switch( DICE_bank->mixAttribute ) {
				...
				if( *dice_factor_ptr == DICE_MIN_FLOWFACTOR  ) {
					// Direction pivot
					...
					*dice_factor_ptr = direction == INC ? 0 : 18;
					...
					modify_parameter( 	dice_factor_ptr, DICE_MIN_FLOWFACTOR, DICE_MAX_FLOWFACTOR, direction, OFF, FIXED	);
				} else {
					modify_parameter( 	dice_factor_ptr, DICE_MAX_FLOWFACTOR, DICE_MAX_FLOWFACTOR << 1, 3 - direction, OFF, FIXED	);
				}
