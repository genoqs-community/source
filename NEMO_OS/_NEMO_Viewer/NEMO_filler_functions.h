...
void MIR_write_base12_H( int value, unsigned char target_row ) {
	...
	if ( sign == NEG ) MIR[LHS][target_row][MIR_BLINK] |= bitpattern;
	...
	if ( fullsteps > 8 ){
		...
		if ( sign == NEG ) MIR[RHS][target_row][MIR_BLINK] |= bitpattern;
	}
	...
	if (gate <= 8) {
		...
		if (gate != fullsteps ){
			...
			if ( sign == NEG ) MIR[LHS][target_row][MIR_BLINK] &= ( 0xFF ^ (1 << (gate-1)) );
		}
	...
	if (gate >= 9) {
		...
		if (gate != fullsteps ){
			...
			if ( sign == NEG ) MIR[RHS][target_row][MIR_BLINK] &= ( 0xFF ^ (1 << (gate-8-1)) );
		}
...
//// OCTOPUS version: writes the mutepattern of the given page
...
// Writes the mutepattern of the given page - The visible tracks only!
void GRID_write_mutepattern(	Pagestruct* target_page, unsigned char target_row ){
	...
	if ( 	( CHECK_BIT( G_on_the_measure_mod_bit, row ) )
		...
		if( CHECK_BIT( G_on_the_measure_operation[row], OPERATION_MUTE ) ) {
			...
			MIR_blink_trackpattern( mirror( ( mutePattern & ( NEMO_MAX_WINDOW )), 16 ), target_row );
		}
		...
		if( CHECK_BIT( G_on_the_measure_operation[row], OPERATION_SOLO ) ) {
			...
			MIR_blink_trackpattern( mirror( ( soloPattern & ( NEMO_MAX_WINDOW )), 16 ), target_row );
			MIR_augment_trackpattern( mirror( ( soloPattern & ( NEMO_MAX_WINDOW )), 16 ), target_row, MIR_GREEN );
			MIR_augment_trackpattern( mirror( ( soloPattern & ( NEMO_MAX_WINDOW )), 16 ), target_row, MIR_RED );
		}
	}
}


//// Writes the mutepattern of the given page - The first 4 tracks only!
//void GRID_write_mutepattern(	Pagestruct* target_page, unsigned char target_row ){
//
//	// OFF tracks
//	MIR_write_trackpattern(
//		( target_page->trackMutepattern & 0x3C0 ) << 6, target_row, MIR_RED   );
//
//	// ON tracks
//	MIR_write_trackpattern(
//		( (target_page->trackMutepattern ^ 0x3ff) & 0x3C0 ) << 6, target_row, MIR_GREEN );
//
//	// SELECTED tracks
//	MIR_write_trackpattern(
//		( target_page->trackSelection & 0x3C0 ) << 6, target_row, MIR_BLINK   );
//
//	// SOLO tracks
//	MIR_augment_trackpattern(
//		( target_page->trackSolopattern & 0x3C0 ) << 6, target_row, MIR_RED   );
//}
...
void MIR_show_startbar_H( unsigned char number, unsigned char target_row ){
	...
	signed char shift = number - 6;

	MIR_fill_numeric( 3, 6, target_row, MIR_GREEN 	);
	MIR_fill_numeric( 9, 6, target_row, MIR_RED 	);

	// The step is symbolysed by tow orange dots in the middle. A red or green bar indicates start push or pull
	MIR_write_dot( 88 + (11 * shift), MIR_GREEN );
	MIR_write_dot( 88 + (11 * shift), MIR_RED );
	MIR_write_dot( 99 + (11 * shift), MIR_GREEN );
	MIR_write_dot( 99 + (11 * shift), MIR_RED );
}
...
// Nemo has no circle, therefore need to code 100 as 10 + rest
...
void MIR_write_length_H_old( unsigned char length, unsigned char target_row ) {
...
void MIR_write_tempo( unsigned char number, unsigned char target_row ){
	...
	if ( hundreds == 1 ){
		...
		// Show the tens LED lit as tens
		MIR_write_dot( 110, tens_color );
	}
