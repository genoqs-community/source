...
//
			...
			MIR_write_trackpattern( Page_get_trackpattern( target_page, j ), 3, MIR_GREEN	    );
			MIR_write_trackpattern( Page_get_skippattern ( target_page, j ), 3, MIR_RED			);
			...
			MIR_augment_trackpattern( Page_get_skippattern(  target_page, j ), 3, MIR_RED   );
			MIR_augment_trackpattern( Page_get_selection_trackpattern( target_page, j ), 3, MIR_BLINK );


			// Blink the Steps whose selection bit is set
			MIR_blink_trackpattern (Page_get_selection_trackpattern(target_page, j), 3);						

			// Track contents of selected Track in rowzero, lauflicht in the selected track
			if (G_run_bit) {
				
				MIR_write_lauflicht_track( j, 3 );
			}
